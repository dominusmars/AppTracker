...
import { jobCollection, mailCollection } from "../data/collections";
...
export class JobUpdate {
  ...
  async saveJobInDatabase() {
    ...
    if (existingJob) {
      ...
      log(
        `Job Status updated: ${this.job} - ${this.company} ${this.status}`,
        "info",
      );
    ...
    log(
      `Job Status inserted: ${this.job} - ${this.company} ${this.status}`,
      "info",
    );
  }
  async deleteJobFromDatabase() {
    ...
    log(
      `Job Status deleted: ${this.job} - ${this.company} ${this.status}`,
      "info",
    );
  }
}
