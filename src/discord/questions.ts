...
class MailClassifyQuestion {
  ...
  async handleJobUpdateInteraction(interaction: ButtonInteraction<CacheType>) {
    ...
    const embed = createClassificationEmbed(
      mail.mail,
      `Mail classified as ${jobUpdate ? "Job Update" : "Regular Mail"}`,
    );

    let classificationMessage = interaction.user.send({ embeds: [embed] });
    // Log the result
    log(
      `Mail ${mail.mail.toString()} classified as ${jobUpdate ? "Job Update" : "Regular Mail"}`,
      "info",
    );

    // Remove the classifcation message after 5 minutes
    setTimeout(
      async () => {
        try {
  ...
  async updateClassifier(mail: Mail, job_update: boolean) {
    ...
    log(
      `Classifier updated with ${mail.subject} ${job_update ? "is Job Update" : "is Regular Mail"}`,
      "debug",
    );
  ...
  async sendDirectMessage(mail: Mail) {
    try {
      ...
      const yes = new ButtonBuilder()
        .setCustomId("jobupdate_yes")
        .setStyle(ButtonStyle.Success)
        .setLabel("Yes");
      const no = new ButtonBuilder()
        .setCustomId("jobupdate_no")
      ...
      log(
        `Error sending message to ${config.DISCORD_USER_ID}: ${error.message}`,
        "error",
      );
