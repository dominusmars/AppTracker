...
function createJobUpdatesListEmbed(JobUpdates: JobUpdate[]) {
  const embed = createDefaultEmbed()
    .setTitle("Job Updates")
    .setDescription("List of job updates")
    .setColor("#00ff00");

  JobUpdates.forEach((jobUpdate) => {
    embed.addFields({
      name: jobUpdate.toString(),
      value: jobUpdate.mail.subject,
    });
  });
...
export {
  createDefaultEmbed,
  createEmailEmbed,
  createClassificationEmbed,
  createJobUpdateEmbed,
  createJobUpdatesListEmbed,
};
