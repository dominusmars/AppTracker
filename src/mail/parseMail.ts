...
const systemMessages: Message[] = [
    {
        role: "system",
        content: `Is this email a job update or job application? If it is, please provide the status, job title, company name and job id, if available. If it is not a job update or job application, please ignore it and return Not a Job Update or Job Application.`,
    },
    {
        role: "system",
...
async function parseMailForAppStatus(
    ...
    function addError(error: AppStatusParsingError) {
        log(`Error parsing for app status ${mail.toString()} ${error.error}`, "debug");
    ...
    try {
        ...
        return parseMailForAppStatus(mail, [...successResults, result], errorResults);
    } catch (e) {
        // Error parsing json
